...
class RAM(nn.Module):
    ...
    def forward(self, inputs):
        ...
        memory = self.locationed_memory(memory, memory_len, left_len, aspect_len).double()
        
        aspect = self.embed(aspect_indices)
        aspect = torch.sum(aspect, dim=1)
        aspect = torch.div(aspect, nonzeros_aspect.unsqueeze(-1)).double()
        et = torch.zeros_like(aspect).double().to(self.opt.device)

        batch_size = memory.size(0)
        seq_len = memory.size(1)
        for _ in range(self.opt.hops):
            g = self.att_linear(torch.cat([memory.double(), 
                torch.zeros(batch_size, seq_len, self.opt.embed_dim,dtype=torch.float64).to(self.opt.device) + et.double().unsqueeze(1), 
                torch.zeros(batch_size, seq_len, self.opt.embed_dim,dtype=torch.float64).to(self.opt.device) + aspect.double().unsqueeze(1)], 
                dim=-1).double())
            alpha = F.softmax(g.double(), dim=1)
