...
class TNet_LF(nn.Module):
    def __init__(self, embedding_matrix, opt):
        ...
        self.convs3 = nn.Conv1d(2 * HD, 50, 3, padding=1).double()
        self.fc1 = nn.Linear(4 * HD, 2 * HD).double()
        self.fc = nn.Linear(50, C).double()
    ...
    def forward(self, inputs):
        ...
        v = v.transpose(1, 2).double()
        e = e.transpose(1, 2).double()
        for i in range(2):
            a = torch.bmm(e.transpose(1, 2), v)
            a = F.softmax(a, 1)  # (aspect_len,context_len)
            aspect_mid = torch.bmm(e, a).double()
            aspect_mid = torch.cat((aspect_mid, v), dim=1).transpose(1, 2).double()
            aspect_mid = F.relu(self.fc1(aspect_mid.double()).transpose(1, 2))
            v = aspect_mid + v
            v = self.position(v.transpose(1, 2), aspect_in_text).transpose(1, 2)
        z = F.relu(self.convs3(v.double()).double())  # [(N,Co,L), ...]*len(Ks)
