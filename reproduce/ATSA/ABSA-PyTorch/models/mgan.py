    ...
    def forward(self, batch_size, ctx, asp):
        ...
        for i, ctx_chunk in enumerate(ctx_chunks):
            for j, asp_chunk in enumerate(asp_chunks):
                ...
                # Added by Abbey 11-04
                feat = feat.double()

                # print()
                # print(type(batch_size))
                # print()
                alignment_mat[:, i, j] = feat.matmul((self.w_u.expand(batch_size, -1, -1)).double()).squeeze(-1).squeeze(-1) 
...
class MGAN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        f_asp2ctx = torch.matmul((ctx_out.transpose(1, 2)).double(), (F.softmax(alignment_mat.max(2, keepdim=True)[0], dim=1)).double()).squeeze(-1)
        f_ctx2asp = torch.matmul((F.softmax(alignment_mat.max(1, keepdim=True)[0], dim=2)).double(), (asp_out).double()).transpose(1, 2).squeeze(-1) 

        c_asp2ctx_alpha = F.softmax(ctx_out.matmul((self.w_a2c.expand(batch_size, -1, -1)).double()).matmul((asp_pool).double()), dim=1)
        c_asp2ctx = torch.matmul(ctx_out.transpose(1, 2), c_asp2ctx_alpha).squeeze(-1)
        c_ctx2asp_alpha = F.softmax((asp_out).double().matmul((self.w_c2a.expand(batch_size, -1, -1)).double()).matmul((ctx_pool).double()), dim=1)
        c_ctx2asp = torch.matmul(asp_out.float().transpose(1, 2), c_ctx2asp_alpha.float()).squeeze(-1)

        feat = torch.cat([c_asp2ctx, f_asp2ctx, f_ctx2asp, c_ctx2asp], dim=1)
        out = self.dense(feat.float()) # bathc_size x polarity_dim
